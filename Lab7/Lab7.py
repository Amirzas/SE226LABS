...
class Vehicle:
    def __init__(onur, vid, model, year):
        onur.vid = vid
        onur.model = model
        onur.year = year
...
class Car(Vehicle):
    def __init__(atakan, vid, model, year, fuel_type, doors):
        super().__init__(vid, model, year)
        atakan.fuel_type = fuel_type
        atakan.doors = int(doors)
...
class Truck(Vehicle):
    def __init__(mirza, vid, model, year, max_load, axles):
        super().__init__(vid, model, year)
        mirza.max_load = int(max_load)
        mirza.axles = int(axles)
...
def load_fleet_from_file(filename):
    ...
    with open(filename, "r") as f:
        for line in f:
            p = line.strip().split(",")
            if p[0] == "Car":
                vehicles.append(Car(p[1], p[2], int(p[3]), p[4], p[5]))
            elif p[0] == "Truck":
                vehicles.append(Truck(p[1], p[2], int(p[3]), p[4], p[5]))
            elif p[0] == "Motorcycle":
                vehicles.append(Motorcycle(p[1], p[2], int(p[3]), p[4], p[5]))
    return vehicles
